  ...
  if(!new_data) {
    ...
    for(let item of __item_data) {
      ...
      if(search_input.value === '') {
        ...
        location.href =  `http://127.0.0.1:5500/Shopping_application/Detail/?Detail?id=${item.id}=/?Detail?imgsrc=${item.image}=/?Detail?name=${item.itemName}=/?Detail?price=${item.price}`
        })
